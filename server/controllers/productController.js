...
exports.createProduct = asyncHandler(async (req, res, next) => {
  try {
    ...
    // Handle brand logo
    const brandLogoResult = await cloudinary.v2.uploader.upload(req.body.logo, {
      folder: 'brands',
    });

    // Parse specifications (if coming as JSON strings)
    const specs = (req.body.specifications || []).map((s) =>
      typeof s === 'string' ? JSON.parse(s) : s
    );

    // Parse highlights (in case sent as comma-separated string or array of strings)
    const highlights = Array.isArray(req.body.highlights)
      ? req.body.highlights
      : (req.body.highlights || '').split(',').map((h) => h.trim());

    // Construct full product body
    const productBody = {
      name: req.body.name,
